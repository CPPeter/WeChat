...
- (NSArray *)emailAddresses {
    ...
    NSXMLElement *array = [emailE elementForName:@"USERID"];
    NSString *email = [array stringValue];
    DLOG(@"emailE = %@",emailE);
    DLOG(@"arrya==%@",array);
    DLOG(@"string=%@",[array stringValue]);
    if(email.length > 0)
    {
        return @[email];
    }
//   return @[[[[self elementForName:@"EMAIL"] elementForName:@"USERID"] stringValue]];
    return nil;
 ...
 */
    ...
    //1.获取EMAIL标签
    NSXMLElement *elementE = [self elementForName:@"EMAIL"];
    //2.获取USERID标签
    NSXMLElement *elementU = [elementE elementForName:@"USERID"];
    
    //3.更改USERID标签的内容
    //移除USERID标签,再添加一个新的
    [elementE removeElementForName:@"USERID"];
    
    
    NSXMLElement *NewELementU = [[NSXMLElement alloc]initWithName:@"USERID" stringValue:[emails firstObject]];
    //添加到elementE
    [elementE addChild:NewELementU];
